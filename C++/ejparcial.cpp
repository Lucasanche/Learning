...
class JefeArea{
    ...
    public:
        ...
        bool grabarDeDisco(){
            ...
            bool leyo=fwrite(this, sizeof(JefeArea),1, p);
            fclose(p);
            return leyo;
        }
...
void puntoB(){
    ...
    while(reg.leerDeDisco(pos)==true){
        if(reg.getTipoContrato()==1 && reg.getFechaContrato().getAnio()==2015){
                vContratos[reg.getFechaContrato().getMes()-1]++;
        }
        pos++;
    }
    int posMax=buscarMaximo(vContratos, 12);
    cout<<"EL MES CON MAS CONTRATOS TEMPORARIOS EN 2015 FUE: "<<posMax+1<<endl;
}
